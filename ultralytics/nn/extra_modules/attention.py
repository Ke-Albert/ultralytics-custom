...
class EMA(nn.Module):
    """
    https://doi.org/10.1038/s41598-025-86239-w
    EMA(Efficient multi-scal Attention) implementation.
...
class AgentAttention(nn.Module):
    def __init__(self, dim, num_patches, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        self.ha_bias = nn.Parameter(
            torch.zeros(1, self.num_heads, self.window_size[0], 1, self.agent_num))
        self.wa_bias = nn.Parameter(
            torch.zeros(1, self.num_heads, 1, self.window_size[1], self.agent_num))
